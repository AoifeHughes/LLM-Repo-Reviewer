...
class TestLibrarian:
    ...
    def test_initialization(self, mock_chromadb, mock_embeddings):
        ...
        with patch('LLMLibrarian.librarian.OpenAI') as mock_openai:
            librarian = Librarian()
    ...
    def test_file_hash_calculation(self, temp_directory, mock_chromadb, mock_embeddings):
    ...
    def test_file_caching(self, temp_directory, mock_chromadb, mock_embeddings):
        ...
        with patch('LLMLibrarian.librarian.OpenAI'):
            ...
            librarian._update_file_cache(test_file, file_hash, ["chunk1", "chunk2"], {"test": "metadata"})
    ...
    def test_query_without_tools(self, mock_chromadb, mock_embeddings, mock_openai_client):
        """Test querying without tool usage"""
        with patch('LLMLibrarian.librarian.OpenAI', return_value=mock_openai_client):
            librarian = Librarian()
            
            response = librarian.query("What is the purpose of this code?", use_tools=False)
    ...
    def test_find_files_tool(self, temp_directory, mock_chromadb, mock_embeddings):
        ...
        with patch('LLMLibrarian.librarian.OpenAI'):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                result = librarian._tool_find_files(path=temp_directory, name_pattern="*.py")
    ...
    def test_grep_content_tool(self, temp_directory, mock_chromadb, mock_embeddings):
        ...
        with patch('LLMLibrarian.librarian.OpenAI'):
            ...
            with patch('subprocess.run') as mock_run:
                ...
                result = librarian._tool_grep_content(pattern="hello", path=temp_directory)
